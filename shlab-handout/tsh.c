...
void sigchld_handler(int sig) {
    ...
    int stat;
    pid_t pid;
    while ((pid = waitpid(-1, &stat, WNOHANG | WUNTRACED)) != -1) {
        // a child process has terminated or stoped
        if (WIFEXITED(stat)) {
            // exit normally
            deletejob(jobs, pid);
...
void sigint_handler(int sig) {
    ...
    // no fg process to kill
    if (pid == 0) {
        return;
    }

    // kill fg process and its group member(sub processes)
    kill(-pid, sig);
